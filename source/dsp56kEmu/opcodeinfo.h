...
namespace dsp56k
{
	...
	struct OpcodeInfo
	{
		constexpr OpcodeInfo(Instruction _inst, const char(&_opcode)[25], const char* _assembly, ExtensionWordTypes _extensionWordType = None)
			: m_instruction(_inst)
			, m_opcode(_opcode)
	...
	{
		OpcodeInfo(Abs,				"????????????????0010d110",	"ABS D", EffectiveAddress),
		OpcodeInfo(ADC,				"????????????????001Jd001",	"ADC S,D", EffectiveAddress),

		OpcodeInfo(Add_SD,			"????????????????0JJJd000",	"ADD S,D", EffectiveAddress),
		OpcodeInfo(Add_xx,			"0000000101iiiiii10ood000",	"ADD #xx,D"),
		OpcodeInfo(Add_xxxx,		"0000000101ooooo011ood000",	"ADD #xxxx,D", ImmediateData),
		OpcodeInfo(Addl,			"????????????????0001d010",	"ADDL S,D", EffectiveAddress),
		OpcodeInfo(Addr,			"????????????????0000d010",	"ADDR S,D", EffectiveAddress),

		OpcodeInfo(And_SD,			"????????????????01JJd110",	"AND S,D", EffectiveAddress),
		...
		OpcodeInfo(Asl_D,			"????????????????0011d010",	"ASL D", EffectiveAddress),
		OpcodeInfo(Asl_ii,			"0000110000011101SiiiiiiD",	"ASL #ii,S2,D"),
		OpcodeInfo(Asl_S1S2D,		"0000110000011110010SsssD",	"ASL S1,S2,D"),

		OpcodeInfo(Asr_D,			"????????????????0010d010",	"ASR D", EffectiveAddress),
		OpcodeInfo(Asr_ii,			"0000110000011100SiiiiiiD",	"ASR #ii,S2,D"),
		OpcodeInfo(Asr_S1S2D,		"0000110000011110011SsssD",	"ASR S1,S2,D"),

		OpcodeInfo(Bcc_xxxx,		"00001101000101000000CCCC",	"Bcc xxxx", EffectiveAddress),	// TODO: documentation is a bit weird here, is the address now part of an extension word or part of aaaa...? Lots of typos in there
		...
		OpcodeInfo(Clr,				"????????????????0001d011",	"CLR D", EffectiveAddress),

		OpcodeInfo(Cmp_S1S2,		"????????????????0JJJd101",	"CMP S1, S2", EffectiveAddress),
		...
		OpcodeInfo(Cmpm_S1S2,		"????????????????0JJJd111",	"CMPM S1, S2", EffectiveAddress),
		...
		OpcodeInfo(Eor_SD,			"????????????????01JJd011",	"EOR S,D", EffectiveAddress),
		...
		OpcodeInfo(Ifcc,			"001000000010CCCC????????",	"IFcc"),
		OpcodeInfo(Ifcc_U,			"001000000011CCCC????????",	"IFcc.U"),
		...
		OpcodeInfo(Jsclr_ea,		"0000101101MMMRRR1S0bbbbb",	"JSCLR #n,[X or Y]:ea,xxxx", EffectiveAddress),
		...
		OpcodeInfo(Jset_ea,			"0000101001MMMRRR1S1bbbbb",	"JSET #n,[X or Y]:ea,xxxx", EffectiveAddress),
		...
		OpcodeInfo(Jsset_ea,		"0000101101MMMRRR1S1bbbbb",	"JSSET #n,[X or Y]:ea,xxxx", EffectiveAddress),
		...
		OpcodeInfo(Lsl_D,			"????????????????0011D011",	"LSL D", EffectiveAddress),
		...
		OpcodeInfo(Mac_S1S2,		"????????????????1QQQdk10",	"MAC (+/-)S1,S2,D / MAC (+/-)S2,S1,D", EffectiveAddress),
		...
		OpcodeInfo(Macr_S1S2,		"????????????????1QQQdk11",	"MACR (+/-)S1,S2,D / MACR (+/-)S2,S1,D", EffectiveAddress),
		...
		OpcodeInfo(Max,				"????????????????00011101",	"MAX A, B", EffectiveAddress),
		OpcodeInfo(Maxm,			"????????????????00010101",	"MAXM A, B", EffectiveAddress),
		...
		OpcodeInfo(Mpy_S1S2D,		"????????????????1QQQdk00",	"MPY (+/-)S1,S2,D / MPY (+/-)S2,S1,D", EffectiveAddress),
		...
		OpcodeInfo(Mpyr_S1S2D,		"????????????????1QQQdk01",	"MPYR (+/-)S1,S2,D / MPYR (+/-)S2,S1,D", EffectiveAddress),
		...
		OpcodeInfo(Neg,				"????????????????0011d110",	"NEG D", EffectiveAddress),
		...
		OpcodeInfo(Not,				"????????????????0001d111",	"NOT D", EffectiveAddress),

		OpcodeInfo(Or_SD,			"????????????????01JJd010",	"OR S,D", EffectiveAddress),
		...
		OpcodeInfo(Pflush,			"000000000000000000000011",	"PFLUSH"),
		OpcodeInfo(Pflushun,		"000000000000000000000001",	"PFLUSHUN"),
		...
		OpcodeInfo(Rnd,				"????????????????0001d001",	"RND D", EffectiveAddress),
		OpcodeInfo(Rol,				"????????????????0011d111",	"ROL D", EffectiveAddress),
		OpcodeInfo(Ror,				"????????????????0010d111",	"ROR D", EffectiveAddress),
		OpcodeInfo(Rti,				"000000000000000000000100",	"RTI"),
		OpcodeInfo(Rts,				"000000000000000000001100",	"RTS"),

		OpcodeInfo(Sbc,				"????????????????001Jd101",	"SBC S,D", EffectiveAddress),

		OpcodeInfo(Stop,			"00000000000000001o0o0111",	"STOP"),

		OpcodeInfo(Sub_SD,			"????????????????0JJJd100",	"SUB S,D", EffectiveAddress),
		OpcodeInfo(Sub_xx,			"0000000101iiiiii10ood100",	"SUB #xx,D"),
		OpcodeInfo(Sub_xxxx,		"0000000101ooooo011ood100",	"SUB #xxxx,D", ImmediateData),

		OpcodeInfo(Subl,			"????????????????0001d110",	"SUBL S,D", EffectiveAddress),
		OpcodeInfo(subr,			"????????????????0000d110",	"SUBR S,D", EffectiveAddress),

		OpcodeInfo(Tcc_S1D1,		"00000010CCCC0ooo0JJJdooo",	"Tcc S1,D1"),
		OpcodeInfo(Tcc_S1D1S2D2,	"00000011CCCCottt0JJJdTTT",	"Tcc S1,D1 S2,D2"),
		OpcodeInfo(Tcc_S2D2,		"00000010CCCC1ttt0ooooTTT",	"Tcc S2,D2"),

		OpcodeInfo(Tfr,				"????????????????0JJJd001",	"TFR S,D", EffectiveAddress),
		OpcodeInfo(Trap,			"000000000000000000000110",	"TRAP"),
		OpcodeInfo(Trapcc,			"00000000000000000001CCCC",	"TRAPcc"),

		OpcodeInfo(Tst,				"????????????????0000d011",	"TST S", EffectiveAddress),
